...
class cuser_list extends cuser {
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->_userid->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fuserlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_user\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_user',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fuserlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
fuserlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php
	...
	if ($bSelectLimit) {
		if ($user_list->TotalRecs <= 0)
			$user_list->TotalRecs = $user->ListRecordCount();
	} else {
...
<form name="fuserlistsrch" id="fuserlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
...
<div class="box ewBox ewGrid<?php if ($user_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> user">
<?php if ($user->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($user->CurrentAction <> "gridadd" && $user->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($user_list->Pager)) $user_list->Pager = new cPrevNextPager($user_list->StartRec, $user_list->DisplayRecs, $user_list->TotalRecs, $user_list->AutoHidePager) ?>
...
<?php if ($user_list->TotalRecs > 0 || $user->CurrentAction == "gridedit") { ?>
...
<?php if ($user->_userid->Visible) { // userid ?>
	<?php if ($user->SortUrl($user->_userid) == "") { ?>
		<th data-name="_userid" class="<?php echo $user->_userid->HeaderCellClass() ?>"><div id="elh_user__userid" class="user__userid"><div class="ewTableHeaderCaption"><?php echo $user->_userid->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="_userid" class="<?php echo $user->_userid->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $user->SortUrl($user->_userid) ?>',1);"><div id="elh_user__userid" class="user__userid">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $user->_userid->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($user->_userid->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($user->_userid->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($user->username->Visible) { // username ?>
	...
	<?php } else { ?>
		<th data-name="username" class="<?php echo $user->username->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $user->SortUrl($user->username) ?>',1);"><div id="elh_user_username" class="user_username">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $user->username->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($user->username->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($user->username->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($user->password->Visible) { // password ?>
	...
	<?php } else { ?>
		<th data-name="password" class="<?php echo $user->password->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $user->SortUrl($user->password) ?>',1);"><div id="elh_user_password" class="user_password">
...
?>
	<?php if ($user->_userid->Visible) { // userid ?>
		<td data-name="_userid"<?php echo $user->_userid->CellAttributes() ?>>
<span id="el<?php echo $user_list->RowCnt ?>_user__userid" class="user__userid">
