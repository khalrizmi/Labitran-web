...
$temuan_kerusakan_edit = NULL; // Initialize page object first

class ctemuan_kerusakan_edit extends ctemuan_kerusakan {
	...
	// Page object name
	var $PageObjName = 'temuan_kerusakan_edit';
	...
	function ValidateForm() {
		...
		if (in_array("temuan_kerusakan_foto", $DetailTblVar) && $GLOBALS["temuan_kerusakan_foto"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// d_temuan
			$this->d_temuan->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->d_temuan->CurrentValue, 1), ew_CurrentDate(), $this->d_temuan->ReadOnly);

			// kerusakan
			$this->kerusakan->SetDbValueDef($rsnew, $this->kerusakan->CurrentValue, NULL, $this->kerusakan->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("temuan_kerusakan_foto", $DetailTblVar) && $GLOBALS["temuan_kerusakan_foto"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["temuan_kerusakan_foto_grid"]->GridUpdate();
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("temuan_kerusakan_foto", $DetailTblVar)) {
				...
				if ($GLOBALS["temuan_kerusakan_foto_grid"]->DetailEdit) {
					$GLOBALS["temuan_kerusakan_foto_grid"]->CurrentMode = "edit";
					$GLOBALS["temuan_kerusakan_foto_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($temuan_kerusakan_edit)) $temuan_kerusakan_edit = new ctemuan_kerusakan_edit();

// Page init
$temuan_kerusakan_edit->Page_Init();

// Page main
$temuan_kerusakan_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$temuan_kerusakan_edit->Page_Render();
?>
...
var CurrentForm = ftemuan_kerusakanedit = new ew_Form("ftemuan_kerusakanedit", "edit");

// Validate form
ftemuan_kerusakanedit.Validate = function() {
...
ftemuan_kerusakanedit.Form_CustomValidate = 
...
ftemuan_kerusakanedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftemuan_kerusakanedit.Lists["x_nik"] = {"LinkField":"x_nik","Ajax":true,"AutoFill":false,"DisplayFields":["x_nik","x_nama","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"karyawan"};
ftemuan_kerusakanedit.Lists["x_nik"].Data = "<?php echo $temuan_kerusakan_edit->nik->LookupFilterQuery(FALSE, "edit") ?>";
ftemuan_kerusakanedit.Lists["x_kerusakan[]"] = {"LinkField":"x_c_kerusakan","Ajax":true,"AutoFill":false,"DisplayFields":["x_c_kerusakan","x_n_kerusakan","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"kerusakan"};
ftemuan_kerusakanedit.Lists["x_kerusakan[]"].Data = "<?php echo $temuan_kerusakan_edit->kerusakan->LookupFilterQuery(FALSE, "edit") ?>";
...
<?php $temuan_kerusakan_edit->ShowPageHeader(); ?>
<?php
$temuan_kerusakan_edit->ShowMessage();
?>
<form name="ftemuan_kerusakanedit" id="ftemuan_kerusakanedit" class="<?php echo $temuan_kerusakan_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($temuan_kerusakan_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $temuan_kerusakan_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="temuan_kerusakan">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($temuan_kerusakan_edit->IsModal) ?>">
<div class="ewEditDiv"><!-- page* -->
<?php if ($temuan_kerusakan->nik->Visible) { // nik ?>
	<div id="r_nik" class="form-group">
		<label id="elh_temuan_kerusakan_nik" for="x_nik" class="<?php echo $temuan_kerusakan_edit->LeftColumnClass ?>"><?php echo $temuan_kerusakan->nik->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $temuan_kerusakan_edit->RightColumnClass ?>"><div<?php echo $temuan_kerusakan->nik->CellAttributes() ?>>
...
<?php if ($temuan_kerusakan->d_temuan->Visible) { // d_temuan ?>
	<div id="r_d_temuan" class="form-group">
		<label id="elh_temuan_kerusakan_d_temuan" for="x_d_temuan" class="<?php echo $temuan_kerusakan_edit->LeftColumnClass ?>"><?php echo $temuan_kerusakan->d_temuan->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $temuan_kerusakan_edit->RightColumnClass ?>"><div<?php echo $temuan_kerusakan->d_temuan->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("ftemuan_kerusakanedit", "x_d_temuan", {"ignoreReadonly":true,"useCurrent":false,"format":1});
</script>
<?php } ?>
</span>
...
<?php if ($temuan_kerusakan->kerusakan->Visible) { // kerusakan ?>
	<div id="r_kerusakan" class="form-group">
		<label id="elh_temuan_kerusakan_kerusakan" for="x_kerusakan" class="<?php echo $temuan_kerusakan_edit->LeftColumnClass ?>"><?php echo $temuan_kerusakan->kerusakan->FldCaption() ?></label>
		<div class="<?php echo $temuan_kerusakan_edit->RightColumnClass ?>"><div<?php echo $temuan_kerusakan->kerusakan->CellAttributes() ?>>
...
<?php if ($temuan_kerusakan->keterangan->Visible) { // keterangan ?>
	<div id="r_keterangan" class="form-group">
		<label id="elh_temuan_kerusakan_keterangan" for="x_keterangan" class="<?php echo $temuan_kerusakan_edit->LeftColumnClass ?>"><?php echo $temuan_kerusakan->keterangan->FldCaption() ?></label>
		<div class="<?php echo $temuan_kerusakan_edit->RightColumnClass ?>"><div<?php echo $temuan_kerusakan->keterangan->CellAttributes() ?>>
...
<input type="hidden" data-table="temuan_kerusakan" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($temuan_kerusakan->id->CurrentValue) ?>">
<?php
	if (in_array("temuan_kerusakan_foto", explode(",", $temuan_kerusakan->getCurrentDetailTable())) && $temuan_kerusakan_foto->DetailEdit) {
?>
...
<?php } ?>
<?php if (!$temuan_kerusakan_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $temuan_kerusakan_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $temuan_kerusakan_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
ftemuan_kerusakanedit.Init();
</script>
<?php
$temuan_kerusakan_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$temuan_kerusakan_edit->Page_Terminate();
?>
