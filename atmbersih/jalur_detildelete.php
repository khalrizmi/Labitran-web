...
$jalur_detil_delete = NULL; // Initialize page object first

class cjalur_detil_delete extends cjalur_detil {
	...
	// Page object name
	var $PageObjName = 'jalur_detil_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("jalur_detillist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in jalur_detil class, jalur_detilinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("jalur_detillist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($jalur_detil_delete)) $jalur_detil_delete = new cjalur_detil_delete();

// Page init
$jalur_detil_delete->Page_Init();

// Page main
$jalur_detil_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$jalur_detil_delete->Page_Render();
?>
...
var CurrentForm = fjalur_detildelete = new ew_Form("fjalur_detildelete", "delete");

// Form_CustomValidate event
fjalur_detildelete.Form_CustomValidate = 
...
fjalur_detildelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fjalur_detildelete.Lists["x_c_bank"] = {"LinkField":"x_c_bank","Ajax":true,"AutoFill":false,"DisplayFields":["x_n_bank","","",""],"ParentFields":[],"ChildFields":["x_atmid"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"bank"};
fjalur_detildelete.Lists["x_c_bank"].Data = "<?php echo $jalur_detil_delete->c_bank->LookupFilterQuery(FALSE, "delete") ?>";
fjalur_detildelete.Lists["x_atmid"] = {"LinkField":"x_atmid","Ajax":true,"AutoFill":false,"DisplayFields":["x_atmid","x_n_atm","x_lokasi",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"atm"};
fjalur_detildelete.Lists["x_atmid"].Data = "<?php echo $jalur_detil_delete->atmid->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $jalur_detil_delete->ShowPageHeader(); ?>
<?php
$jalur_detil_delete->ShowMessage();
?>
<form name="fjalur_detildelete" id="fjalur_detildelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($jalur_detil_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $jalur_detil_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="jalur_detil">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($jalur_detil_delete->RecKeys as $key) { ?>
...
<?php if ($jalur_detil->c_bank->Visible) { // c_bank ?>
		<th class="<?php echo $jalur_detil->c_bank->HeaderCellClass() ?>"><span id="elh_jalur_detil_c_bank" class="jalur_detil_c_bank"><?php echo $jalur_detil->c_bank->FldCaption() ?></span></th>
<?php } ?>
<?php if ($jalur_detil->atmid->Visible) { // atmid ?>
		<th class="<?php echo $jalur_detil->atmid->HeaderCellClass() ?>"><span id="elh_jalur_detil_atmid" class="jalur_detil_atmid"><?php echo $jalur_detil->atmid->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$jalur_detil_delete->RecCnt = 0;
$i = 0;
while (!$jalur_detil_delete->Recordset->EOF) {
	$jalur_detil_delete->RecCnt++;
	$jalur_detil_delete->RowCnt++;

	// Set row properties
	$jalur_detil->ResetAttrs();
	$jalur_detil->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$jalur_detil_delete->LoadRowValues($jalur_detil_delete->Recordset);

	// Render row
	$jalur_detil_delete->RenderRow();
?>
	<tr<?php echo $jalur_detil->RowAttributes() ?>>
<?php if ($jalur_detil->c_bank->Visible) { // c_bank ?>
		<td<?php echo $jalur_detil->c_bank->CellAttributes() ?>>
<span id="el<?php echo $jalur_detil_delete->RowCnt ?>_jalur_detil_c_bank" class="jalur_detil_c_bank">
...
<?php } ?>
<?php if ($jalur_detil->atmid->Visible) { // atmid ?>
		<td<?php echo $jalur_detil->atmid->CellAttributes() ?>>
<span id="el<?php echo $jalur_detil_delete->RowCnt ?>_jalur_detil_atmid" class="jalur_detil_atmid">
...
<?php
	$jalur_detil_delete->Recordset->MoveNext();
}
$jalur_detil_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $jalur_detil_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fjalur_detildelete.Init();
</script>
<?php
$jalur_detil_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$jalur_detil_delete->Page_Terminate();
?>
