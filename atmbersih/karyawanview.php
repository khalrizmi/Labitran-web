...
$karyawan_view = NULL; // Initialize page object first

class ckaryawan_view extends ckaryawan {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["nik"] <> "") {
			$this->RecKey["nik"] = $_GET["nik"];
			$KeyUrl .= "&amp;nik=" . urlencode($this->RecKey["nik"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["nik"] <> "") {
				...
				$this->nik->setFormValue($_POST["nik"]);
				$this->RecKey["nik"] = $this->nik->FormValue;
			} else {
				$sReturnUrl = "karyawanlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "karyawanlist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "karyawanlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($karyawan_view)) $karyawan_view = new ckaryawan_view();

// Page init
$karyawan_view->Page_Init();

// Page main
$karyawan_view->Page_Main();
...
var CurrentForm = fkaryawanview = new ew_Form("fkaryawanview", "view");

// Form_CustomValidate event
fkaryawanview.Form_CustomValidate = 
...
fkaryawanview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fkaryawanview.Lists["x_kode_jabatan"] = {"LinkField":"x_c_jabatan","Ajax":true,"AutoFill":false,"DisplayFields":["x_n_jabatan","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"jabatan"};
fkaryawanview.Lists["x_kode_jabatan"].Data = "<?php echo $karyawan_view->kode_jabatan->LookupFilterQuery(FALSE, "view") ?>";
fkaryawanview.AutoSuggests["x_kode_jabatan"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $karyawan_view->kode_jabatan->LookupFilterQuery(TRUE, "view"))) ?>;
fkaryawanview.Lists["x_lokasid"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_n_lokasi","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"lokasi_kerja"};
fkaryawanview.Lists["x_lokasid"].Data = "<?php echo $karyawan_view->lokasid->LookupFilterQuery(FALSE, "view") ?>";
...
<?php
	foreach ($karyawan_view->OtherOptions as &$option)
		$option->Render("body");
?>
...
?>
<form name="fkaryawanview" id="fkaryawanview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($karyawan_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $karyawan_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="karyawan">
<input type="hidden" name="modal" value="<?php echo intval($karyawan_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($karyawan->nik->Visible) { // nik ?>
	<tr id="r_nik">
		<td class="col-sm-2"><span id="elh_karyawan_nik"><?php echo $karyawan->nik->FldCaption() ?></span></td>
		<td data-name="nik"<?php echo $karyawan->nik->CellAttributes() ?>>
<span id="el_karyawan_nik">
<span<?php echo $karyawan->nik->ViewAttributes() ?>>
<?php echo $karyawan->nik->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($karyawan->kode_jabatan->Visible) { // kode_jabatan ?>
	<tr id="r_kode_jabatan">
		<td class="col-sm-2"><span id="elh_karyawan_kode_jabatan"><?php echo $karyawan->kode_jabatan->FldCaption() ?></span></td>
...
<?php echo $karyawan->kode_jabatan->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($karyawan->nama->Visible) { // nama ?>
	<tr id="r_nama">
		<td class="col-sm-2"><span id="elh_karyawan_nama"><?php echo $karyawan->nama->FldCaption() ?></span></td>
...
<?php echo $karyawan->nama->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($karyawan->alamat->Visible) { // alamat ?>
	<tr id="r_alamat">
		<td class="col-sm-2"><span id="elh_karyawan_alamat"><?php echo $karyawan->alamat->FldCaption() ?></span></td>
...
<?php echo $karyawan->alamat->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($karyawan->telp->Visible) { // telp ?>
	<tr id="r_telp">
		<td class="col-sm-2"><span id="elh_karyawan_telp"><?php echo $karyawan->telp->FldCaption() ?></span></td>
...
<?php echo $karyawan->telp->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($karyawan->lokasid->Visible) { // lokasid ?>
	<tr id="r_lokasid">
		<td class="col-sm-2"><span id="elh_karyawan_lokasid"><?php echo $karyawan->lokasid->FldCaption() ?></span></td>
...
<?php echo $karyawan->lokasid->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
