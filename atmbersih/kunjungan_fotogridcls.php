...
$kunjungan_foto_grid = NULL; // Initialize page object first

class ckunjungan_foto_grid extends ckunjungan_foto {
	...
	// Page object name
	var $PageObjName = 'kunjungan_foto_grid';

	// Grid form hidden field names
	var $FormName = 'fkunjungan_fotogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["kunjungan_foto"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->kunjunganid->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@id@", ew_AdjustSql($this->kunjunganid->getSessionValue(), "DB"), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// kunjunganid
		if ($this->kunjunganid->getSessionValue() <> "") {
			$rsnew['kunjunganid'] = $this->kunjunganid->getSessionValue();
		}
		if ($this->foto->Visible && !$this->foto->Upload->KeepFile) {
			$OldFiles = ew_Empty($this->foto->Upload->DbValue) ? array() : array($this->foto->Upload->DbValue);
			if (!ew_Empty($this->foto->Upload->FileName)) {
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "kunjungan") {
			$this->kunjunganid->Visible = FALSE;
			if ($GLOBALS["kunjungan"]->EventCancelled) $this->EventCancelled = TRUE;
		}
