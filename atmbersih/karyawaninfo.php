...
//
// Table class for karyawan
//
class ckaryawan extends cTable {
	var $nik;
	var $kode_jabatan;
	var $nama;
	var $alamat;
	...
	function __construct() {
		...
		$this->TableVar = 'karyawan';
		$this->TableName = 'karyawan';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`karyawan`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// nik
		$this->nik = new cField('karyawan', 'karyawan', 'x_nik', 'nik', '`nik`', '`nik`', 200, -1, FALSE, '`nik`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nik->Sortable = TRUE; // Allow sort
		$this->fields['nik'] = &$this->nik;

		// kode_jabatan
		$this->kode_jabatan = new cField('karyawan', 'karyawan', 'x_kode_jabatan', 'kode_jabatan', '`kode_jabatan`', '`kode_jabatan`', 200, -1, FALSE, '`kode_jabatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kode_jabatan->Sortable = TRUE; // Allow sort
		$this->fields['kode_jabatan'] = &$this->kode_jabatan;

		// nama
		$this->nama = new cField('karyawan', 'karyawan', 'x_nama', 'nama', '`nama`', '`nama`', 200, -1, FALSE, '`nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama->Sortable = TRUE; // Allow sort
		$this->fields['nama'] = &$this->nama;

		// alamat
		$this->alamat = new cField('karyawan', 'karyawan', 'x_alamat', 'alamat', '`alamat`', '`alamat`', 200, -1, FALSE, '`alamat`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->alamat->Sortable = TRUE; // Allow sort
		$this->fields['alamat'] = &$this->alamat;

		// telp
		$this->telp = new cField('karyawan', 'karyawan', 'x_telp', 'telp', '`telp`', '`telp`', 200, -1, FALSE, '`telp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->telp->Sortable = TRUE; // Allow sort
		$this->fields['telp'] = &$this->telp;

		// lokasid
		$this->lokasid = new cField('karyawan', 'karyawan', 'x_lokasid', 'lokasid', '`lokasid`', '`lokasid`', 3, -1, FALSE, '`lokasid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->lokasid->Sortable = TRUE; // Allow sort
		$this->lokasid->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->lokasid->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->lokasid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['lokasid'] = &$this->lokasid;

		// foto
		$this->foto = new cField('karyawan', 'karyawan', 'x_foto', 'foto', '`foto`', '`foto`', 200, -1, TRUE, '`foto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`karyawan`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('nik', $rs))
				ew_AddFilter($where, ew_QuotedName('nik', $this->DBID) . '=' . ew_QuotedValue($rs['nik'], $this->nik->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (is_null($this->nik->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@nik@", ew_AdjustSql($this->nik->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "karyawanedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "karyawanadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("karyawanview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("karyawanview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "karyawanadd.php?" . $this->UrlParm($parm);
		else
			$url = "karyawanadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("karyawanedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("karyawanadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("karyawandelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "nik:" . ew_VarToJson($this->nik->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->nik->CurrentValue)) {
			$sUrl .= "nik=" . urlencode($this->nik->CurrentValue);
		} else {
	...
	function LoadListRowValues(&$rs) {
		...
		$this->kode_jabatan->setDbValue($rs->fields('kode_jabatan'));
		$this->nama->setDbValue($rs->fields('nama'));
		$this->alamat->setDbValue($rs->fields('alamat'));
		$this->telp->setDbValue($rs->fields('telp'));
		$this->lokasid->setDbValue($rs->fields('lokasid'));
	...
	function RenderEditRow() {
		...
		// lokasid
		$this->lokasid->EditAttrs["class"] = "form-control";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->kode_jabatan->Exportable) $Doc->ExportCaption($this->kode_jabatan);
					if ($this->nama->Exportable) $Doc->ExportCaption($this->nama);
					if ($this->alamat->Exportable) $Doc->ExportCaption($this->alamat);
					if ($this->telp->Exportable) $Doc->ExportCaption($this->telp);
					if ($this->lokasid->Exportable) $Doc->ExportCaption($this->lokasid);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->kode_jabatan->Exportable) $Doc->ExportField($this->kode_jabatan);
						if ($this->nama->Exportable) $Doc->ExportField($this->nama);
						if ($this->alamat->Exportable) $Doc->ExportField($this->alamat);
						if ($this->telp->Exportable) $Doc->ExportField($this->telp);
						if ($this->lokasid->Exportable) $Doc->ExportField($this->lokasid);
