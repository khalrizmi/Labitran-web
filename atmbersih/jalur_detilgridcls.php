...
$jalur_detil_grid = NULL; // Initialize page object first

class cjalur_detil_grid extends cjalur_detil {
	...
	// Page object name
	var $PageObjName = 'jalur_detil_grid';

	// Grid form hidden field names
	var $FormName = 'fjalur_detilgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["jalur_detil"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_c_bank") && $objForm->HasValue("o_c_bank") && $this->c_bank->CurrentValue <> $this->c_bank->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_atmid") && $objForm->HasValue("o_atmid") && $this->atmid->CurrentValue <> $this->atmid->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->c_bank->setOldValue($objForm->GetValue("o_c_bank"));
		if (!$this->atmid->FldIsDetailKey) {
			$this->atmid->setFormValue($objForm->GetValue("x_atmid"));
		}
		$this->atmid->setOldValue($objForm->GetValue("o_atmid"));
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->c_jalur->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@c_jalur@", ew_AdjustSql($this->c_jalur->getSessionValue(), "DB"), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		...
		// c_jalur
		if ($this->c_jalur->getSessionValue() <> "") {
			$rsnew['c_jalur'] = $this->c_jalur->getSessionValue();
		}
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "jalur") {
			$this->c_jalur->Visible = FALSE;
			if ($GLOBALS["jalur"]->EventCancelled) $this->EventCancelled = TRUE;
