...
$temuan_kerusakan_foto_grid = NULL; // Initialize page object first

class ctemuan_kerusakan_foto_grid extends ctemuan_kerusakan_foto {
	...
	// Page object name
	var $PageObjName = 'temuan_kerusakan_foto_grid';

	// Grid form hidden field names
	var $FormName = 'ftemuan_kerusakan_fotogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["temuan_kerusakan_foto"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->temuan_kerusakan_id->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@id@", ew_AdjustSql($this->temuan_kerusakan_id->getSessionValue(), "DB"), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// temuan_kerusakan_id
		if ($this->temuan_kerusakan_id->getSessionValue() <> "") {
			$rsnew['temuan_kerusakan_id'] = $this->temuan_kerusakan_id->getSessionValue();
		}
		if ($this->foto->Visible && !$this->foto->Upload->KeepFile) {
			$OldFiles = ew_Empty($this->foto->Upload->DbValue) ? array() : array($this->foto->Upload->DbValue);
			if (!ew_Empty($this->foto->Upload->FileName)) {
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "temuan_kerusakan") {
			$this->temuan_kerusakan_id->Visible = FALSE;
			if ($GLOBALS["temuan_kerusakan"]->EventCancelled) $this->EventCancelled = TRUE;
		}
