...
$atm_edit = NULL; // Initialize page object first

class catm_edit extends catm {
	...
	function LoadFormValues() {
		...
		if (!$this->areaid->FldIsDetailKey) {
			$this->areaid->setFormValue($objForm->GetValue("x_areaid"));
		}
	...
	function RestoreFormValues() {
		...
		$this->areaid->CurrentValue = $this->areaid->FormValue;
	...
	function RenderRow() {
		...
		$this->lontitude->ViewCustomAttributes = "";
			...
			$this->areaid->EditValue = ew_HtmlEncode($this->areaid->CurrentValue);
			if (strval($this->areaid->CurrentValue) <> "") {
				$sFilterWrk = "`id`" . ew_SearchString("=", $this->areaid->CurrentValue, EW_DATATYPE_NUMBER, "");
			$sSqlWrk = "SELECT `id`, `n_area` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `area`";
			$sWhereWrk = "";
			...
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				...
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$arwrk = array();
					$arwrk[1] = ew_HtmlEncode($rswrk->fields('DispFld'));
					$this->areaid->EditValue = $this->areaid->DisplayValue($arwrk);
					$rswrk->Close();
				} else {
					$this->areaid->EditValue = ew_HtmlEncode($this->areaid->CurrentValue);
				}
			} else {
				$this->areaid->EditValue = NULL;
			}
	...
	function ValidateForm() {
		...
		if (!ew_CheckInteger($this->areaid->FormValue)) {
			ew_AddMessage($gsFormError, $this->areaid->FldErrMsg());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// c_cabang
			$this->c_cabang->SetDbValueDef($rsnew, $this->c_cabang->CurrentValue, NULL, $this->c_cabang->ReadOnly);

			// c_bmi
			$this->c_bmi->SetDbValueDef($rsnew, $this->c_bmi->CurrentValue, NULL, $this->c_bmi->ReadOnly);

			// atmid
			// n_atm

			$this->n_atm->SetDbValueDef($rsnew, $this->n_atm->CurrentValue, NULL, $this->n_atm->ReadOnly);

			// lokasi
			$this->lokasi->SetDbValueDef($rsnew, $this->lokasi->CurrentValue, NULL, $this->lokasi->ReadOnly);

			// lokasi_sebelum
			$this->lokasi_sebelum->SetDbValueDef($rsnew, $this->lokasi_sebelum->CurrentValue, NULL, $this->lokasi_sebelum->ReadOnly);

			// areaid
			$this->areaid->SetDbValueDef($rsnew, $this->areaid->CurrentValue, NULL, $this->areaid->ReadOnly);

			// kotaid
			$this->kotaid->SetDbValueDef($rsnew, $this->kotaid->CurrentValue, NULL, $this->kotaid->ReadOnly);

			// latitude
			$this->latitude->SetDbValueDef($rsnew, $this->latitude->CurrentValue, NULL, $this->latitude->ReadOnly);

			// lontitude
			$this->lontitude->SetDbValueDef($rsnew, $this->lontitude->CurrentValue, NULL, $this->lontitude->ReadOnly);
	...
	function SetupLookupFilters($fld, $pageId = null) {
		...
		case "x_areaid":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id` AS `LinkFld`, `n_area` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `area`";
			$sWhereWrk = "{filter}";
			$fld->LookupFilters = array("dx1" => '`n_area`');
	...
	function SetupAutoSuggestFilters($fld, $pageId = null) {
		...
		case "x_areaid":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id`, `n_area` AS `DispFld` FROM `area`";
			$sWhereWrk = "`n_area` LIKE '{query_value}%'";
			$fld->LookupFilters = array("dx1" => '`n_area`');
...
// Create page object
if (!isset($atm_edit)) $atm_edit = new catm_edit();
...
var CurrentForm = fatmedit = new ew_Form("fatmedit", "edit");

// Validate form
fatmedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			elm = this.GetElements("x" + infix + "_areaid");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($atm->areaid->FldErrMsg()) ?>");
...
fatmedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fatmedit.Lists["x_c_bank"] = {"LinkField":"x_c_bank","Ajax":true,"AutoFill":false,"DisplayFields":["x_c_bank","x_n_bank","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"bank"};
fatmedit.Lists["x_c_bank"].Data = "<?php echo $atm_edit->c_bank->LookupFilterQuery(FALSE, "edit") ?>";
fatmedit.Lists["x_areaid"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_n_area","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"area"};
fatmedit.Lists["x_areaid"].Data = "<?php echo $atm_edit->areaid->LookupFilterQuery(FALSE, "edit") ?>";
fatmedit.AutoSuggests["x_areaid"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $atm_edit->areaid->LookupFilterQuery(TRUE, "edit"))) ?>;
fatmedit.Lists["x_kotaid"] = {"LinkField":"x_kabupatenid","Ajax":true,"AutoFill":false,"DisplayFields":["x_lokasi_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"kabupaten"};
fatmedit.Lists["x_kotaid"].Data = "<?php echo $atm_edit->kotaid->LookupFilterQuery(FALSE, "edit") ?>";
...
<?php $atm_edit->ShowPageHeader(); ?>
<?php
$atm_edit->ShowMessage();
?>
<form name="fatmedit" id="fatmedit" class="<?php echo $atm_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($atm_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $atm_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="atm">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($atm_edit->IsModal) ?>">
<div class="ewEditDiv"><!-- page* -->
<?php if ($atm->c_bank->Visible) { // c_bank ?>
	<div id="r_c_bank" class="form-group">
		<label id="elh_atm_c_bank" for="x_c_bank" class="<?php echo $atm_edit->LeftColumnClass ?>"><?php echo $atm->c_bank->FldCaption() ?></label>
		<div class="<?php echo $atm_edit->RightColumnClass ?>"><div<?php echo $atm->c_bank->CellAttributes() ?>>
...
<?php if ($atm->c_cabang->Visible) { // c_cabang ?>
	<div id="r_c_cabang" class="form-group">
		<label id="elh_atm_c_cabang" for="x_c_cabang" class="<?php echo $atm_edit->LeftColumnClass ?>"><?php echo $atm->c_cabang->FldCaption() ?></label>
		<div class="<?php echo $atm_edit->RightColumnClass ?>"><div<?php echo $atm->c_cabang->CellAttributes() ?>>
...
<?php if ($atm->c_bmi->Visible) { // c_bmi ?>
	<div id="r_c_bmi" class="form-group">
		<label id="elh_atm_c_bmi" for="x_c_bmi" class="<?php echo $atm_edit->LeftColumnClass ?>"><?php echo $atm->c_bmi->FldCaption() ?></label>
		<div class="<?php echo $atm_edit->RightColumnClass ?>"><div<?php echo $atm->c_bmi->CellAttributes() ?>>
...
<?php if ($atm->atmid->Visible) { // atmid ?>
	<div id="r_atmid" class="form-group">
		<label id="elh_atm_atmid" for="x_atmid" class="<?php echo $atm_edit->LeftColumnClass ?>"><?php echo $atm->atmid->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $atm_edit->RightColumnClass ?>"><div<?php echo $atm->atmid->CellAttributes() ?>>
<span id="el_atm_atmid">
<span<?php echo $atm->atmid->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $atm->atmid->EditValue ?></p></span>
</span>
<input type="hidden" data-table="atm" data-field="x_atmid" name="x_atmid" id="x_atmid" value="<?php echo ew_HtmlEncode($atm->atmid->CurrentValue) ?>">
<?php echo $atm->atmid->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($atm->n_atm->Visible) { // n_atm ?>
	<div id="r_n_atm" class="form-group">
		<label id="elh_atm_n_atm" for="x_n_atm" class="<?php echo $atm_edit->LeftColumnClass ?>"><?php echo $atm->n_atm->FldCaption() ?></label>
		<div class="<?php echo $atm_edit->RightColumnClass ?>"><div<?php echo $atm->n_atm->CellAttributes() ?>>
...
<?php if ($atm->lokasi->Visible) { // lokasi ?>
	<div id="r_lokasi" class="form-group">
		<label id="elh_atm_lokasi" for="x_lokasi" class="<?php echo $atm_edit->LeftColumnClass ?>"><?php echo $atm->lokasi->FldCaption() ?></label>
		<div class="<?php echo $atm_edit->RightColumnClass ?>"><div<?php echo $atm->lokasi->CellAttributes() ?>>
...
<?php if ($atm->lokasi_sebelum->Visible) { // lokasi_sebelum ?>
	<div id="r_lokasi_sebelum" class="form-group">
		<label id="elh_atm_lokasi_sebelum" for="x_lokasi_sebelum" class="<?php echo $atm_edit->LeftColumnClass ?>"><?php echo $atm->lokasi_sebelum->FldCaption() ?></label>
		<div class="<?php echo $atm_edit->RightColumnClass ?>"><div<?php echo $atm->lokasi_sebelum->CellAttributes() ?>>
...
<?php } ?>
<?php if ($atm->areaid->Visible) { // areaid ?>
	<div id="r_areaid" class="form-group">
		<label id="elh_atm_areaid" class="<?php echo $atm_edit->LeftColumnClass ?>"><?php echo $atm->areaid->FldCaption() ?></label>
		<div class="<?php echo $atm_edit->RightColumnClass ?>"><div<?php echo $atm->areaid->CellAttributes() ?>>
<span id="el_atm_areaid">
<?php
$wrkonchange = trim(" " . @$atm->areaid->EditAttrs["onchange"]);
if ($wrkonchange <> "") $wrkonchange = " onchange=\"" . ew_JsEncode2($wrkonchange) . "\"";
$atm->areaid->EditAttrs["onchange"] = "";
?>
<span id="as_x_areaid" style="white-space: nowrap; z-index: 8920">
	<input type="text" name="sv_x_areaid" id="sv_x_areaid" value="<?php echo $atm->areaid->EditValue ?>" size="30" placeholder="<?php echo ew_HtmlEncode($atm->areaid->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($atm->areaid->getPlaceHolder()) ?>"<?php echo $atm->areaid->EditAttributes() ?>>
</span>
<input type="hidden" data-table="atm" data-field="x_areaid" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $atm->areaid->DisplayValueSeparatorAttribute() ?>" name="x_areaid" id="x_areaid" value="<?php echo ew_HtmlEncode($atm->areaid->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
fatmedit.CreateAutoSuggest({"id":"x_areaid","forceSelect":false});
</script>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($atm->areaid->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_areaid',m:0,n:10,srch:true});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($atm->areaid->ReadOnly || $atm->areaid->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
</span>
<?php echo $atm->areaid->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($atm->kotaid->Visible) { // kotaid ?>
	<div id="r_kotaid" class="form-group">
		<label id="elh_atm_kotaid" for="x_kotaid" class="<?php echo $atm_edit->LeftColumnClass ?>"><?php echo $atm->kotaid->FldCaption() ?></label>
		<div class="<?php echo $atm_edit->RightColumnClass ?>"><div<?php echo $atm->kotaid->CellAttributes() ?>>
...
<?php if ($atm->latitude->Visible) { // latitude ?>
	<div id="r_latitude" class="form-group">
		<label id="elh_atm_latitude" for="x_latitude" class="<?php echo $atm_edit->LeftColumnClass ?>"><?php echo $atm->latitude->FldCaption() ?></label>
		<div class="<?php echo $atm_edit->RightColumnClass ?>"><div<?php echo $atm->latitude->CellAttributes() ?>>
...
<?php if ($atm->lontitude->Visible) { // lontitude ?>
	<div id="r_lontitude" class="form-group">
		<label id="elh_atm_lontitude" for="x_lontitude" class="<?php echo $atm_edit->LeftColumnClass ?>"><?php echo $atm->lontitude->FldCaption() ?></label>
		<div class="<?php echo $atm_edit->RightColumnClass ?>"><div<?php echo $atm->lontitude->CellAttributes() ?>>
...
<?php if (!$atm_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $atm_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $atm_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
